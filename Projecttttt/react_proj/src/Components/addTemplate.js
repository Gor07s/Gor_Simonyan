...
class AddTemplate extends Component {
    ...
    async check () {
        if (document.getElementById("templateName").value === "" ||
            ...
            await this.setState({url: "/addTemplate"});
            alert("Pls, write anything in lines Template and Text! " + this.state.url);
            return false;
        }
        for (let i = 1; i <= this.state.rId; i++) {
            if (!(/^\w+([-]?\w+)*@\w+([-]?\w+)*(\.\w{2,3})+$/.test(document.getElementById("recipients" + i).value)) && document.getElementById("recipients" + i).value !== "") {
